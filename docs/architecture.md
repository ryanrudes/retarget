...
flowchart LR
  ...
  problem --> engine[InteractionMeshRetargetingEngine]
  engine --> result[RetargetingResult]
  result --> eval[EvaluationReport]
...
1. Applies `output_fps` by resampling the source motion and dynamic object trajectory when requested.
2. Scales the input motion to the target robot when requested.
3. Builds an interaction mesh from mapped human joints and object, terrain, or ground sample points.
4. Computes source Laplacian coordinates in the task frame.
5. Queries a kinematics backend for robot point positions and Jacobians.
6. Lowers registered objective and constraint terms into a local quadratic subproblem over actuated-joint increments.
7. Solves the subproblem, including joint limits, trust regions, inferred foot-contact locking, explicit foot-lock windows, ground non-penetration, scene clearance, and backend-provided self-collision separation when enabled.
